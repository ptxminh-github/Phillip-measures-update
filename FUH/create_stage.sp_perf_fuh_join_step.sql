...
IF OBJECT_ID('[tmp].[sp_perf_fuh_join_step]', 'P') IS NOT NULL
DROP PROCEDURE [tmp].[sp_perf_fuh_join_step];
GO
CREATE PROCEDURE [tmp].[sp_perf_fuh_join_step]
...
LEFT JOIN [tmp].[v_perf_fuh_inpatient_index_stay] AS b
...
FROM [tmp].[v_perf_fuh_follow_up_visit]
